...
class BIOMASSAuxPP1Structure:
    ...
    def __set_product_paths(self):
        ...
        if os.path.exists(self.product_path):
            # Set paths of BIOMASS AUX_PP1 product single files starting from an existing one
            # - MPH file
            self.mph_file = (glob(os.path.join(self.product_path, "*.xml")) or [None])[0]
            # - Parameters file
            self.parameters_file = (glob(os.path.join(self.product_path, self.data_subfolder, "*_pp1.xml")) or [None])[
                0
            ]
            # - Schema files
