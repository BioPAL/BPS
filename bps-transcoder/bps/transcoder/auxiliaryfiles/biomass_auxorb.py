...
class BIOMASSAuxOrbStructure:
    ...
    def __set_product_paths(self):
        ...
        if os.path.exists(self.product_path):
            # Set paths of BIOMASS AUX_ORB product single files starting from an existing one
            # - MPH file
            self.mph_file = (glob(os.path.join(self.product_path, "*.xml")) or [None])[0]
            # - Parameters file
            self.parameters_file = (glob(os.path.join(self.product_path, self.data_subfolder, "*_orb*.xml")) or [None])[
                0
            ]
            # - Schema files
            self.schema_files = glob(os.path.join(self.product_path, self.schema_subfolder, "*.xsd"))
        else:
            # Set paths of BIOMASS AUX_ORB product single files starting from product name
            ...
            self.parameters_file = (
                os.path.join(self.product_path, self.data_subfolder, product_name.lower()[:-10]) + "_orb.xml"
            )
            # - Schema files
            self.schema_files = [
