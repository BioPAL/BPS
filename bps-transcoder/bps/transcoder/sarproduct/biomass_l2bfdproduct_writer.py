...
from bps.transcoder import BPS_ATBD_FD_VERSION, BPS_PFD_FD_VERSION, BPS_PPD_VERSION
from bps.transcoder.io import (
    common_annotation_models_l2,
    main_annotation_models_l2b_fd,
    vrt,
)
...
class BIOMASSL2bFDProductWriter:
    """_summary_"""

    def __init__(
        self,
        product: BIOMASSL2bFDProduct,  # product to write
        ...
        footprint_masks_for_quicklooks: dict[str, np.ndarray],
    ) -> None:
        """_summary_

        Parameters
        ----------
        product : BIOMASSL2bFDProduct
            _description_
        product_path : _type_
            _description_
        processor_name : str
        ...
        self.footprint_masks_for_quicklooks = footprint_masks_for_quicklooks
    ...
    def _write_mph_file(self):
        ...
        files = (
            [
                (self.product_structure.fd_file, None),
                (
                ...
                (
                    ...
                    self.product_structure.l2b_fd_main_ann_xsd,
                ),
            ]
        ...
        xml4.text = L2B_OUTPUT_PRODUCT_FD
        ...
        xml4 = ET.SubElement(xml3, "{" + MPH_NAMESPACES["bio"] + "}refDoc")
        xml4.text = (
            f"BIOMASS Forest Disturbance Algorithms Theoretical Baseline Document (BPS_FD_ATBD) {BPS_ATBD_FD_VERSION}"
        )
    ...
    def _write_stac_file(self):
        ...
        properties = {
            ...
            "processing:product_type": "Forest Disturbance",
            ...
            "proj:shape": list(self.product.measurement.data_dict["fd"].shape),
        }
        ...
        assets_dict["fd"] = pystac.asset.Asset(
            href="./" + str(Path(self.product_structure.fd_file).relative_to(product_root)),
        ...
        assets_dict["probability"] = pystac.asset.Asset(
            href="./" + str(Path(self.product_structure.probability_file).relative_to(product_root)),
        ...
        assets_dict["cfm"] = pystac.asset.Asset(
            href="./" + str(Path(self.product_structure.cfm_file).relative_to(product_root)),
        ...
        assets_dict["quicklook_fd"] = pystac.asset.Asset(
            href="./" + str(Path(preview_file_root + "_fd_ql.png").relative_to(product_root)),
        ...
        assets_dict["quicklook_probability"] = pystac.asset.Asset(
            href="./" + str(Path(preview_file_root + "_probability_ql.png").relative_to(product_root)),
        ...
        assets_dict["quicklook_cfm"] = pystac.asset.Asset(
            href="./"
            + str(Path(preview_file_root + self.product.name.lower()[:-10] + "_cfm_ql.png").relative_to(product_root)),
    ...
    def _write_measurement_files_core(
        ...
        if isinstance(data, dict):
            # its the heat_map contributing + agreeing case
            data_to_write = [data["heat_map_contributing"], data["heat_map_agreeing"]]
        else:
            if len(data.shape) == 3:
                # its the heat_map
                data_to_write = []
    ...
    def _write_measurement_files(self):
        ...
        assert self.product_structure.fd_file is not None
        assert self.product_structure.probability_file is not None
        assert self.product_structure.cfm_file is not None
        ...
        for key in self.product.measurement.data_dict.keys():
            ...
            if key == "fd":
                file_path = self.product_structure.fd_file
            elif key == "cfm":
                file_path = self.product_structure.cfm_file
            elif key == "probability_ofchange":
                file_path = self.product_structure.probability_file
                no_data_value = FLOAT_NODATA_VALUE
                key = "probability_of_change"
            elif key == "heat_map":
    ...
    def __write_vrt_file(self):
        ...
        vrtraster_band_list = []
        raster_xsize = self.product.measurement.data_dict["fd"].shape[1]
        raster_ysize = self.product.measurement.data_dict["fd"].shape[0]

        for band, (file_name, key) in enumerate(
            zip(
                [
    ...
    def _write_main_annotation_file(self):
        ...
        footprint = common_annotation_models_l2.FloatArrayWithUnits(
            value=footprint, count=len(self.footprint), units=common_types.UomType.DEG
        )
        ...
        compression_options_fd = self.product.main_ads_processing_parameters.compression_options
    ...
    def write(self):
        ...
        _type_
            _description_
        """
        bps_logger.info("Writing BIOMASS L2b FD product")
