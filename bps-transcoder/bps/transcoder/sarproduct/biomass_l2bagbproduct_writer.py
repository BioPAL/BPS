...
COORDINATE_REFERENCE_SYSTEM = 'GEOGCS["WGS 84",DATUM["WGS_1984",ELLIPSOID["WGS 84",6378137,298.257223563,AUTHORITY["EPSG","7030"]],AUTHORITY["EPSG","6326"]],PRIMEM["Greenwich",0,AUTHORITY["EPSG","8901"]],UNIT["degree",0.0174532925199433,AUTHORITY["EPSG","9122"]],AXIS["Latitude",NORTH],AXIS["Longitude",EAST],AUTHORITY["EPSG","4326"]]'
DECIMATION_FACTOR_QUICKLOOKS = 2
AVERAGING_FACTOR_QUICKLOOKS = 2


class BIOMASSL2bAGBProductWriter:
    """_summary_"""

    def __init__(
        self,
        product: BIOMASSL2bAGBProduct,  # product to write
        ...
        gcp_list: list,
        aux_pp2_agb_name: str,
        cal_ab_product_name: str,
        lcm_product_name: str,
        start_time_l2a: PreciseDateTime,
        stop_time_l2a: PreciseDateTime,
        footprint_mask_for_quicklooks: dict[str, np.ndarray],
        input_l2b_fd_products_names: list[str] | None = None,
        input_l2b_agb_products_names: list[str] | None = None,
    ) -> None:
        """_summary_

        Parameters
        ----------
        product : BIOMASSL2bABProduct
            _description_
        product_path : _type_
            _description_
        processor_name : str
        ...
        footprint_mask_for_quicklooks: dict[str, np.ndarray]
            _description_
        input_l2b_fd_products_names: Optional[List[str]] = None
            _description_
        input_l2b_agb_products_names: Optional[List[str]] = None
            _description_
        """
        ...
        self.aux_pp2_agb_name = aux_pp2_agb_name
        self.cal_ab_product_name = cal_ab_product_name
        self.lcm_product_name = lcm_product_name
        self.input_l2b_fd_products_names = input_l2b_fd_products_names
        self.input_l2b_agb_products_names = input_l2b_agb_products_names
    ...
    def _write_mph_file(self):
        ...
        files = (
            [
                (self.product_structure.agb_file, None),
                (
                ...
                (
                    ...
                    self.product_structure.l2b_agb_main_ann_xsd,
                ),
            ]
        ...
        xml4.text = L2B_OUTPUT_PRODUCT_AGB
        ...
        xml6.text = self.aux_pp2_agb_name
        xml6 = ET.SubElement(xml5, "{" + MPH_NAMESPACES["eop"] + "}auxiliaryDataSetFileName")
        xml6.text = self.cal_ab_product_name
        xml6 = ET.SubElement(xml5, "{" + MPH_NAMESPACES["eop"] + "}auxiliaryDataSetFileName")
        xml6.text = self.lcm_product_name
        if self.input_l2b_fd_products_names:
            for input_l2b_fd_product_name in self.input_l2b_fd_products_names:
                xml6 = ET.SubElement(xml5, "{" + MPH_NAMESPACES["eop"] + "}auxiliaryDataSetFileName")
                xml6.text = input_l2b_fd_product_name
        if self.input_l2b_agb_products_names:
            for input_l2b_agb_product_name in self.input_l2b_agb_products_names:
                xml6 = ET.SubElement(xml5, "{" + MPH_NAMESPACES["eop"] + "}auxiliaryDataSetFileName")
                xml6.text = input_l2b_agb_product_name
        ...
        xml4 = ET.SubElement(xml3, "{" + MPH_NAMESPACES["bio"] + "}agbTileStatus")
        xml4.text = self.product.agb_tile_status.value

        xml4 = ET.SubElement(xml3, "{" + MPH_NAMESPACES["bio"] + "}agbTileIteration")
        xml4.text = str(self.product.agb_tile_iteration)
    ...
    def _write_stac_file(self):
        ...
        properties = {
            ...
            "processing:product_type": "Above Ground Biomass",
            ...
            "proj:shape": list(self.product.measurement.data_dict["agb"].shape),
        }
        ...
        assets_dict["agb"] = pystac.asset.Asset(
            href="./" + str(Path(self.product_structure.agb_file).relative_to(product_root)),
        ...
        assets_dict["agbstandard_deviation"] = pystac.asset.Asset(
            href="./" + str(Path(self.product_structure.agb_standard_deviation_file).relative_to(product_root)),
            title="Above Ground Biomass Standard Deviation",
            description=None,
            media_type=None,
            roles=["measurement", "data"],
            extra_fields={"type": "tiff"},
        ...
        assets_dict["quicklook_agb"] = pystac.asset.Asset(
            href="./" + str(Path(preview_file_root + "_agb_ql.png").relative_to(product_root)),
            title="QuickLook Above Ground Biomass",
            description=None,
            media_type=None,
            roles=["preview", "image"],
            extra_fields={"type": "png"},
        )
        assets_dict["quicklook_agb_std_dev"] = pystac.asset.Asset(
            href="./"
            + str(
                Path(preview_file_root + self.product.name.lower()[:-10] + "_agb_std_dev.png").relative_to(product_root)
            ),
            title="Above Ground Biomass Standard Deviation",
            description=None,
            media_type=None,
    ...
    def _write_measurement_files_core(
        ...
        compression_options : Dict
            Dictionary containing two keys:
                ...
                MAX_Z_ERROR: float (this should be zero in caseo of binary MDS)
            _description_
        ...
        if isinstance(data, dict):
            # its the heat_map contributing + agreeing case
            data_to_write = [
                data["heat_map"],
                data["heat_map_ref_data"],
                data["heat_map_additional_ref_data"],
            ]
        else:
            if "Forest Mask" in cog_metadata.image_description:
                # logic OR or the many FNF present

                # initialize
                no_datavalues_mask = data[:, :, 0] == INT_NODATA_VALUE
                fnf_final = deepcopy(data[:, :, 0])
                fnf_final[fnf_final == INT_NODATA_VALUE] = 0
                # Cycle over all masks
                num_fnf = data.shape[2]
                for fnf_idx in range(num_fnf):
                    no_datavalues_mask_curr = data[:, :, fnf_idx] == INT_NODATA_VALUE
                    no_datavalues_mask = np.logical_and(no_datavalues_mask, no_datavalues_mask_curr)

                    fnf_curr = deepcopy(data[:, :, fnf_idx])
                    fnf_curr[fnf_curr == INT_NODATA_VALUE] = 0
                    fnf_final = np.logical_or(fnf_final, fnf_curr).astype(np.uint8)

                fnf_final[no_datavalues_mask] = INT_NODATA_VALUE
                data = fnf_final
    ...
    def _write_measurement_files(self):
        ...
        assert self.product_structure.agb_file is not None
        assert self.product_structure.agb_standard_deviation_file is not None
        ...
        for key in self.product.measurement.data_dict.keys():
            ...
            if key == "agb":
                file_path = self.product_structure.agb_file
            elif key == "agbstandard_deviation":
                file_path = self.product_structure.agb_standard_deviation_file
            elif key == "heat_map":
    ...
    def __write_vrt_file(self):
        ...
        vrtraster_band_list = []
        raster_xsize = self.product.measurement.data_dict["agb"].shape[1]
        raster_ysize = self.product.measurement.data_dict["agb"].shape[0]
    ...
    def _write_main_annotation_file(self):
        ...
        first_latitude_value = translate_common.translate_float_with_unit_to_model(
            float(self.product.main_ads_raster_image.first_latitude_value),
            common.UomType.DEG,
        )
        first_longitude_value = translate_common.translate_float_with_unit_to_model(
            float(self.product.main_ads_raster_image.first_longitude_value),
            common.UomType.DEG,
        )
        latitude_spacing = translate_common.translate_float_with_unit_to_model(
            float(self.product.main_ads_raster_image.latitude_spacing),
            common.UomType.DEG,
        )
        longitude_spacing = translate_common.translate_float_with_unit_to_model(
            float(self.product.main_ads_raster_image.longitude_spacing),
            common.UomType.DEG,
        )
        ...
        backscatter_limits = main_annotation_models_l2b_agb.BackscatterLimitsType(
            hh=self.product.main_ads_processing_parameters.backscatter_limits[
                main_annotation_models_l2b_agb.PolarisationType.HH.value
            ],
            vh=self.product.main_ads_processing_parameters.backscatter_limits[
                main_annotation_models_l2b_agb.PolarisationType.HV.value
            ],
            vv=self.product.main_ads_processing_parameters.backscatter_limits[
                main_annotation_models_l2b_agb.PolarisationType.VV.value
            ],
        )
    ...
    def _write_quicklook_files(self):
        ...
        tile_id = str(dirname.parent)[-22:-15]
        ...
        for key in self.product.measurement.data_dict.keys():
            ...
            if key in ["agb", "agbstandard_deviation"]:
                imm_to_save[imm_to_save == FLOAT_NODATA_VALUE] = 0.0

                if key == "agb":
                    name_to_search = "_agb_ql.png"
                else:
                    name_to_search = "_agb_std_dev_ql.png"

            elif key == "heat_map":
                imm_to_save1 = self.product.measurement.data_dict[key]["heat_map"]
                imm_to_save2 = self.product.measurement.data_dict[key]["heat_map_ref_data"]
                imm_to_save3 = self.product.measurement.data_dict[key]["heat_map_additional_ref_data"]
                imm_to_save1[imm_to_save1 == INT_NODATA_VALUE] = 0
                imm_to_save2[imm_to_save2 == INT_NODATA_VALUE] = 0
                imm_to_save3[imm_to_save3 == INT_NODATA_VALUE] = 0

                name_to_search1 = "_hm_ql.png"
                name_to_search2 = "_hm_reference_ql.png"
                name_to_search3 = "_hm_additional_reference_ql.png"
            ...
            if AVERAGING_FACTOR_QUICKLOOKS > 1:
                ...
                if key == "heat_map":
                    ...
                    imm_to_save3 = np.sqrt(
                        convolve2d(imm_to_save3**2, boxcar, "same")
                        / (AVERAGING_FACTOR_QUICKLOOKS * AVERAGING_FACTOR_QUICKLOOKS)
                    )[::DECIMATION_FACTOR_QUICKLOOKS, ::DECIMATION_FACTOR_QUICKLOOKS]
            ...
            if key == "heat_map":
                full_fname1 = None
                for full_fname1 in self.product_structure.quicklook_files:
                    if name_to_search1 in full_fname1:
                        break
                assert full_fname1 is not None

                full_fname2 = None
                for full_fname2 in self.product_structure.quicklook_files:
                    if name_to_search2 in full_fname2:
                        break
                assert full_fname2 is not None

                full_fname3 = None
                for full_fname3 in self.product_structure.quicklook_files:
                    if name_to_search3 in full_fname3:
                        break
                ...
                imm_to_save1 = imm_to_save1.astype(np.uint8)
                imm_to_save2 = imm_to_save2.astype(np.uint8)
                imm_to_save3 = imm_to_save3.astype(np.uint8)
                ...
                alpha = np.where(self.footprint_mask_for_quicklooks[tile_id], 255, 0).astype(np.uint8)
                ...
                # Insert Alpha channel to the image BRGA
                imm_bgra2[:, :, 3] = alpha
                cv2.imwrite(full_fname2, imm_bgra2)

                # Convert from 2D GRAY image a 4D BGRA (replica of image in three RGB channels)
                imm_bgra3 = cv2.cvtColor(imm_to_save3, cv2.COLOR_GRAY2BGRA)
                # make transparent (alpha = 0) out of the footprint
                alpha = np.where(self.footprint_mask_for_quicklooks[tile_id], 255, 0).astype(np.uint8)
                # Insert Alpha channel to the image BRGA
                imm_bgra3[:, :, 3] = alpha
                cv2.imwrite(full_fname3, imm_bgra3)
    ...
    def write(self):
        ...
        _type_
            _description_
        """
        bps_logger.info("Writing BIOMASS L2b AGB product")
