...
from bps.stack_processor.cli import cli
...
def main() -> int:
    """Call command line interface and intercepts the error code"""

    try:
        cli()  # pylint: disable=no-value-for-parameter
    except SystemExit as exit_exc:
        if exit_exc.code == 0:
            bps_logger.info("%s correctly terminated", BPS_STACK_PROCESSOR_NAME)
            return SUCCESS_CODE
    ...
    bps_logger.error("%s abnormal termination", BPS_STACK_PROCESSOR_NAME)
    return FAILURE_CODE
