...
└── l0_footprint_generator/
    ├── footprint_generic_start_stop.py         L0 footprint CLI
    ├── generic_isp.py
    ├── isp_stream.py
    ├── isp_structure.py
    └── sar_isp.py
...
- `footprint_generic_start_stop.py` resolves its `BiomassProduct` import
  relative to its own location, so it no longer assumes a specific install
  path.
