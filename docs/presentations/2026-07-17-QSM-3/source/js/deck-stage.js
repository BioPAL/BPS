...
(() => {
  ...
  const stylesheet = `
    ...
    .btn.present .icon-exit { display: none; }
    .btn.present[aria-pressed="true"] .icon-enter { display: none; }
    .btn.present[aria-pressed="true"] .icon-exit { display: block; }
  ...
  class DeckStage extends HTMLElement {
    ...
    _render() {
      ...
      overlay.innerHTML = `
        ...
        <button class="btn present" type="button" aria-label="Present" aria-pressed="false" title="Present (F)">
          <svg class="icon-enter" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M2 6V2h4M10 2h4v4M14 10v4h-4M6 14H2v-4"/></svg>
          <svg class="icon-exit" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 2v4H2M14 6h-4V2M10 14v-4h4M2 10h4v4"/></svg>
        </button>
      ...
      overlay.querySelector('.present').addEventListener('click', () => this._toggleViewerPresent());
    ...
    _railWidth() {
      ...
      if (!this._railEnabled || !this._railVisible || this.hasAttribute('no-rail')
          ...
          || this._viewerPresent || NARROW_MQ.matches) return 0;
    ...
    _syncPresentButton() {
      if (!this._presentBtn) return;
      const on = this._viewerPresent;
      this._presentBtn.setAttribute('aria-pressed', on ? 'true' : 'false');
      this._presentBtn.setAttribute('aria-label', on ? 'Exit present mode' : 'Present');
      this._presentBtn.title = on ? 'Exit present (F or Esc)' : 'Present (F)';
    }

    async _enterViewerPresent() {
      if (this._viewerPresent) return;
      this._viewerPresent = true;
      this._syncPresentButton();
      this._syncRailHidden();
      this._fit();
      this._scaleThumbs();
      this._flashOverlay();
      try {
        ...
        // Fullscreen may be blocked; rail is still hidden for a cleaner view.
      }
    }

    async _exitViewerPresent() {
      if (!this._viewerPresent) return;
      this._viewerPresent = false;
      this._syncPresentButton();
      this._syncRailHidden();
    ...
    _toggleViewerPresent() {
      if (this._viewerPresent) this._exitViewerPresent();
      else this._enterViewerPresent();
    }

    _onFullscreenChange() {
      if (this._isFullscreen() || !this._viewerPresent) return;
      this._viewerPresent = false;
      this._syncPresentButton();
      this._syncRailHidden();
      this._fit();
    ...
    _onKey(e) {
      ...
      if (e.key === 'Escape' && this._viewerPresent) {
        this._exitViewerPresent();
        e.preventDefault();
        return;
      }
