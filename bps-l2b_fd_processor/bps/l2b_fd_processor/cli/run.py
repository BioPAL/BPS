...
from bps.l2b_fd_processor import BPS_L2B_FD_PROCESSOR_NAME, commands
from bps.l2b_fd_processor import __version__ as VERSION
...
def cli(
    ...
    """BIOMASS Processing Suite - L2b FD Processor

    Starts processing from Job Order file
    """
    working_dir_path, job_order_path = processor_setup(
        job_order_file, working_dir, BPS_L2B_FD_PROCESSOR_NAME, "L2B_FD_P", VERSION
    )

    with working_directory(working_dir_path):
        commands.run_l2b_fd_processing(job_order_path, working_dir_path)
