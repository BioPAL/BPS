...
2. Initialize a local conda channel
    ...
    ```sh
    mkdir -p local_l2b_fh_channel && cp -r ./pkgs/* local_l2b_fh_channel && conda index --verbose ${PWD}/local_l2b_fh_channel
    ```
...
3. Configure conda channels priorities
    ...
    ```sh
    conda config --prepend channels conda-forge && conda config --prepend channels ${PWD}/local_l2b_fh_channel
    ```

4. Install the L2B_FH_P Processor
...
#### Execution procedure
The L2B_FH_P Processor can be executed with
```sh
bps_l2b_fh_processor BIO_L2B_P_02.21_0001_20220101T000000_JobOrder.xml
```
